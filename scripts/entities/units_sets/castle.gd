...
var scene = preload("res://scenes/entities/buildings/castle.tscn")  # Path to the house scene
var ghost_preview: Node2D = null

func _ready():
	# Connect signals from the TextureRect (house icon)
	if not $"Panel/banner/castle icon".is_connected("gui_input", Callable(self, "_on_castle_icon_gui_input")):
		$"Panel/banner/castle icon".connect("gui_input",Callable(self, "_on_castle_icon_gui_input"))

	
func _on_castle_icon_gui_input(event: InputEvent) -> void:
