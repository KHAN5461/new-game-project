...
var scene = preload("res://scenes/entities/buildings/wall.tscn")  # Path to the wall scene
var ghost_preview: Node2D = null

func _ready():
	# Connect signals from the TextureRect (wall icon)
	if not $"Panel/banner/wall icon".is_connected("gui_input", Callable(self, "_on_wall_icon_gui_input")):
		$"Panel/banner/wall icon".connect("gui_input",Callable(self, "_on_wall_icon_gui_input"))


	
func _on_wall_icon_gui_input(event):
...
func create_ghost_preview():
	# Create a semi-transparent preview of the wall
...
func _on_area_entered(area):
	if area.name=="tree" or area.name=="wall" or area.name=="tower" or area.name=="castle" or area.name=="limits":
		is_overlapping = true
